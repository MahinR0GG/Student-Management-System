...
export class StudentDashboardComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        // Load real assignments for student's class
        this.loadStudentAssignments(user.className, user.division);

        // Load real marks for student
        this.loadStudentMarks(user.id);

        // Default mock data for attendance and teacher contact
    ...
    // Load real assignments for the student's class
    private loadStudentAssignments(className: string, division: string) {
        if (!className || !division) {
            console.log('No class/division info, skipping assignment load');
            this.allHomeworks = [];
            return;
        }
        ...
        this.api.getAssignmentsByClass(className, division).subscribe({
            next: (response: any) => {
                ...
                this.allHomeworks = assignments.map((a: any) => ({
                ...
                console.log('✅ Assignments loaded:', this.allHomeworks);
                this.filterDashboard();
            },
            error: (err) => {
    ...
    // Load real marks for the student
    private loadStudentMarks(studentId: number) {
        this.api.getMarksByStudent(studentId).subscribe({
            next: (response: any) => {
                const marks = Array.isArray(response) ? response : (response.results || []);
                this.allMarks = marks.map((m: any) => ({
                    subject: m.subject_name || m.subject,
                    score: m.percentage || ((m.marks_obtained / m.total_marks) * 100),
